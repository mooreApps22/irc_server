...
void	CommandHandler::_joinFp(parsed_message& parsed_msg)
{
		...
		chIt != channelNames.end(); ++chIt) //, keyIt = keyNames.begin()
	{
		...
		if (!_srvAPI.isChannelUser(*chIt))
		{
			...
			_srvAPI.send_reply(replyMessage); // TODO send to all members in the channel including user
			...
			std::string message = _srvAPI.getChannelUsersList(*chIt); // TODO get all members but the user and append to the user nick name as it seems that should be the first in the list
